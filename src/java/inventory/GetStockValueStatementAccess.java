 ...
 * @author bhaumik
 */
public class GetStockValueStatementAccess extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (dataConnection != null) {
            try {
                String sql = "select t.branch_cd,SR_NAME,tag_no as IMEI_NO,PUR_RATE,b.BRAND_NAME,MODEL_NAME,case when l.v_date is null then PUR_DATE else l.v_date end as v_date from tag t left join "
                        ...
                        + " where t.is_del = 0 and t.PUR_REF_NO <>'' and IMEI_NO ='' and SERAIL_NO='' ";
                if (mode.equalsIgnoreCase("brand")) {
                    if (!code.equalsIgnoreCase("")) {
                        if (including == 0) {
                            sql += " and m.brand_cd in (" + code + ")";
                        } else {
                            sql += " and m.brand_cd not in (" + code + ")";
                        }
                    }
