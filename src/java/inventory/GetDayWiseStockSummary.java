 ...
 * @author bhaumik
 */
public class GetDayWiseStockSummary extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (dataConnection != null) {
            try {
                String sql = "SELECT SR_ALIAS,t.branch_cd,SR_NAME,SUM(CASE WHEN DATEDIFF( CASE WHEN l.v_date IS NOT NULL THEN l.v_date WHEN t.PUR_DATE IS NOT NULL THEN t.PUR_DATE ELSE '2016-04-01' END ,CURRENT_DATE)*-1 <=15  "
                        + " THEN 1 ELSE 0 END) AS day_15, "
                        + " SUM(CASE WHEN DATEDIFF( CASE WHEN l.v_date IS NOT NULL THEN l.v_date WHEN t.PUR_DATE IS NOT NULL THEN t.PUR_DATE ELSE '2016-04-01' END ,CURRENT_DATE)*-1 <=45 and "
                        + " DATEDIFF( CASE WHEN l.v_date IS NOT NULL THEN l.v_date WHEN t.PUR_DATE IS NOT NULL THEN t.PUR_DATE ELSE '2016-04-01' END ,CURRENT_DATE)*-1 >15 "
                        + " THEN 1 ELSE 0 END )AS day_45, "
                        + " SUM(CASE WHEN DATEDIFF( CASE WHEN l.v_date IS NOT NULL THEN l.v_date WHEN t.PUR_DATE IS NOT NULL THEN t.PUR_DATE ELSE '2016-04-01' END ,CURRENT_DATE)*-1 <=90 and "
                        + " DATEDIFF( CASE WHEN l.v_date IS NOT NULL THEN l.v_date WHEN t.PUR_DATE IS NOT NULL THEN t.PUR_DATE ELSE '2016-04-01' END ,CURRENT_DATE)*-1 >45 "
                        + " THEN 1 ELSE 0 END )AS day_90, "
                        + " SUM(CASE WHEN DATEDIFF( CASE WHEN l.v_date IS NOT NULL THEN l.v_date WHEN t.PUR_DATE IS NOT NULL THEN t.PUR_DATE ELSE '2016-04-01' END ,CURRENT_DATE)*-1 >90 "
                        ...
                        + " LEFT JOIN lbrphd l ON l.ref_no=t.PUR_REF_NO LEFT JOIN acntmst a ON l.ac_cd=a.ac_cd WHERE t.is_del = 0  "
                        + " ";
