 ...
 * @author bhaumik
 */
public class AddUpdatePurchaseReturnBill extends HttpServlet {
        ...
        if (dataConnection != null) {
            try {
                ...
                if (header.getRef_no().equalsIgnoreCase("")) {
                    header.setRef_no(lb.generateKey(dataConnection, "PRHD", "REF_NO", "PR", 7));
                    inv_no = lb.getLast(dataConnection, "INV_NO", "PRHD", "IS_DEL", "0") + 1;
                    ...
                    sql = "INSERT INTO PRHD (INV_NO,V_DATE,V_TYPE,PMT_MODE,AC_CD,DET_TOT,BRANCH_CD,"
                            + "TAX_AMT,ADD_TAX_AMT,NET_AMT,USER_ID,ADJST,ADVANCE_AMT,REMARK,PMT_DAYS,DUE_DATE,REF_NO)"
                            + " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                } else {
                    new PurchaseReturnUpdate().deleteEntry(dataConnection, header.getRef_no());
                    inv_no = Integer.parseInt(lb.getData(dataConnection, "inv_no", "PRHD", "REF_NO", header.getRef_no(), 0));
                    ...
                    pstLocal.executeUpdate();
                    sql = "UPDATE PRHD set INV_NO=?,V_DATE=?,V_TYPE=?,PMT_MODE=?,AC_CD=?"
                ...
                sql = "Update PRHD set INIT_TIMESTAMP = TIME_STAMP where ref_no='" + header.getRef_no() + "'";
                pstLocal = dataConnection.prepareStatement(sql);
                pstLocal.executeUpdate();

                sql = "DELETE FROM PRDT WHERE REF_NO='" + header.getRef_no() + "'";
                pstLocal = dataConnection.prepareStatement(sql);
                pstLocal.executeUpdate();
                ...
                for (int i = 0; i < detail.size(); i++) {
                    sql = "INSERT INTO PRDT (REF_NO,SR_NO,TAG_NO,SR_CD,IMEI_NO,SERAIL_NO,QTY,RATE,DISC_RATE,MRP,AMT,PUR_TAG_NO,TAX_CD,"
                ...
                new PurchaseReturnUpdate().addEntry(dataConnection, header.getRef_no());
