 ...
 * @author bhaumik
 */
public class AveragePurchaseReport extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (dataConnection != null) {
            try {
                String sql = " SELECT b.BRAND_NAME,v.branch_cd,MODEL_NAME,COUNT(t.tag_no) as pcs,SUM(PUR_RATE) AS profit,SUM(PUR_RATE)/COUNT(t.tag_no) AS AVG FROM tag t LEFT JOIN  SERIESMST s ON t.sr_cd=s.sr_cd \n"
                        + " LEFT JOIN MODELMST m ON s.model_cd=m.model_cd LEFT JOIN brandmst b ON m.brand_cd=b.brand_cd LEFT JOIN lbrphd v ON t.PUR_REF_NO = v.REF_NO"
                        + " where v.v_date>='" + from_date + "' and "
                        + " v.v_date<='" + to_date + "'  and v.is_del=0 ";
