 ...
 * @author bhaumik
 */
public class MarginReportSummary extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (dataConnection != null) {
            try {
                String sql = "select v.v_date,v.branch_cd,sum(PUR_RATE) as PUR_RATE,sum(SALE_RATE) as SALE_RATE from tag t left join "
                        + "  VILSHD v on t.SALE_REF_NO = v.REF_NO LEFT JOIN  SERIESMST s ON t.sr_cd=s.sr_cd "
                        + " LEFT JOIN MODELMST m ON s.model_cd=m.model_cd "
                ...
                sql += " group by v_date,branch_cd order by v.v_date,v.v_type,v.inv_no";
...
//                sql += " order by SR_NAME";
                ...
                if (bank_charges) {
                    sql = "SELECT sum(p.card_chg+p.bajaj_chg) as BANK_CHARGES,v.branch_cd from VILSHD v left join payment p on v.ref_no=p.ref_no "
                            + " where v.v_date>='" + from_date + "' and  v.v_date<='" + to_date + "'  and v.is_del=0 ";
